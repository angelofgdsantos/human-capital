...
# PLOTTING
def simulation_plot(s_plot = 5, nb_sim = 100, p = 'nipy_spectral_r', sim_nb = [0,20,40,60,80,99]):
    sims = []
    for s in sim_nb:    
        plot_frame = {}
    ...
    plotname = 'simulation_histories_'+str(s_plot)+'_sim.png'
...
def t_graphs(t_plot = 5, p = 'nipy_spectral_r', ts = [0,10,15,20,25,29]):
    t_frames = []
    for t in ts:
        plot_frame = {}
...
for s in range(sim_nb):
    ...
    for t in range(30):
        ...
        if t != 29:
            ...
            e_shock = np.random.normal(loc = eps_mean, scale = eps_sd)
            w = income_g(Tm,d+1,Tm)
            l = e_shock + income_g(Tm,d,Tm)
            dec = max(w,l)
            u = dec
            if dec == w:
                dec = 0 
...
simulated_data.to_pickle('simulation_'+str(sim_nb)+'.pkl')
simulated_data.to_csv('simulation_'+str(sim_nb)+'.csv', index = False)
